"""
train_model.py — ResNet18 regression for displacement prediction.

Predicts crosshead displacement (mm) from a single video frame.

Usage:
    python displacement_test/train_model.py

Outputs:
    displacement_test/model/best_model.pth   — best validation-loss weights
    displacement_test/model/model_stats.json — label min/max for denormalization
"""
...
CSV_PATH   = os.path.join(BASE_DIR, "dataset", "train", "dataset.csv")
MODEL_DIR  = os.path.join(BASE_DIR, "model")
MODEL_PATH = os.path.join(MODEL_DIR, "best_model.pth")
STATS_PATH = os.path.join(MODEL_DIR, "model_stats.json")

VAL_SPLIT   = 0.20      # fraction of train data used for validation
BATCH_SIZE  = 16
LR_HEAD     = 1e-4
LR_BACKBONE = 1e-5
EPOCHS      = 200
PATIENCE    = 20        # early stopping patience (tighter given small dataset)
...
def main():
    ...
    print(f"Displacement range (train): [{label_min:.4f}, {label_max:.4f}] mm")

    # 4. Datasets & loaders
    train_ds = FrameDataset(train_df, label_min, label_max, augment=True)
    val_ds   = FrameDataset(val_df,   label_min, label_max, augment=False)
    ...
    # 5. Device, model, loss, optimizer, scheduler
    ...
    print(f"\nBest val MAE: {best_val_loss * label_range:.3f} mm")
